<template>
  <div class="bgcolor">
    <!-- banner图 -->
    <div class="p18 mb14"> 
      <img :src="require('@/assets/images/customer/banner@2x.png')" class="radius8">
    </div>
    <!-- 渠道标签 -->
...
<script>
import { addInfoClick, getOne, getInfoByCateId } from '@/api/cms'
  ...
  methods: {
    init() {
      if (this.$route.query.bannerInfoId) {
        getOne(this.$route.query.bannerInfoId)
          .then(res => {
      ...
      if (this.$route.query.tagInfoId) {
        getOne(this.$route.query.tagInfoId)
          .then(res => {
