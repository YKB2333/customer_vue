<template>
  <div id="page">
    <van-tabs @change="changeTab" style="border-bottom:12px solid #f3f4f9" line-width="30px"  color="#3A78EF" v-model="active">
      <van-tab title="销售代理类"></van-tab>
      <van-tab title="购销交易类"></van-tab>
      <van-tab title="一件代发"></van-tab>
    </van-tabs>
  ...
  methods: {
    ...
    getDetail(){
      return new Promise((resolve, reject) => {
        let title = this.active == 0?'销售代理类':this.active==1?'购销交易类':'一件代发'
        cmsSelectPage({
          pageSize: 10,
          infoCategoryId: cmsCategoryId.cooperationAgreement,
          status: '1',
          title,
