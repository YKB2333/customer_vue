        ...
        <ul class="IndustryInformation">
          <template v-for="item in list">
            <li
              v-if="item.source"
              :key="item.id"
              @click="$router.push(`/customerdetails/${item.source}`)"
            >
              <div>
...
export default {
  ...
  methods: {
    handleScroll: function() {
      ...
      if(document.getElementById('page') && document.getElementById('page').scrollTop){
    ...
    onLoad() {
      cmsSelectPage({
        ...
        pageSize: 10,
        infoCategoryId: cmsCategoryId.cooperationCust,
        status: '1',
