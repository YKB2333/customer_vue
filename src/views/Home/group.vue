<template>
  <div id="page">
    ...
    <van-tabs @change="changeTab" style="border-bottom:12px solid #f3f4f9" line-width="30px"  color="#3A78EF" v-model="active">
      <van-tab title="集团简介"></van-tab>
      <van-tab title="集团企业文化"></van-tab>
      <van-tab title="集团优势"></van-tab>
    </van-tabs>
  ...
  methods: {
    ...
    getDetail(){
      return new Promise((resolve, reject) => {
        let title = this.active == 0?'集团简介':this.active==1?'集团企业文化':'集团优势'
        cmsSelectPage({
          pageSize: 10,
          infoCategoryId: cmsCategoryId.group,
          status: '1',
          title,
